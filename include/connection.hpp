...
public:
    ...
    static shared_ptr make_shared(asio::io_service& io_service, asio::ip::tcp::socket socket,
        connection_pool& clients, asio::strand& output_strand);
    void start();
    void send(const async_message::shared_ptr& message);
    void receive(const async_message::shared_ptr& async_message, bool reuse_buffer, const async_message::handle& handle);
