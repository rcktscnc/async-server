...
void connection_pool::receive(std::size_t connection_id, bool reuse_buffer, const async_message::handle& handle)
{
    _container_strand.post([this, connection_id, reuse_buffer, handle = std::move(handle)]()
    {
        auto iterator = std::find_if(_connections.begin(), _connections.end(),
            [connection_id](const _pair_t& e) { return e.first == connection_id; });
        
        if (iterator != _connections.end())
            iterator->second->receive(async_message::make_shared(_output_strand), reuse_buffer, handle);
        else
